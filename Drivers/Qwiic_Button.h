...
#ifndef __Qwiic_Button_H__
#define __Qwiic_Button_H__

#include <stdint.h>

#include "Qwiic_Button_registers.h"
...
class QwiicButton : public i2c_RPi
{
public:
    //Device status
    bool begin(uint8_t address = DEFAULT_ADDRESS); //Sets device I2C address to a user-specified address, over whatever port the user specifies.
    ...
    uint8_t writeDoubleRegisterWithReadback(Qwiic_Button_Register reg, uint16_t data);  //Writes data into a double (two 8-bit) registers, and checks to make sure that the data was written successfully. Returns 0 on no error, 1 on I2C write fail, and 2 if the register doesn't read back the same value that was written.
};
#endif // __Qwiic_Button_H__